        ...
        <div class="modal-body">
          <form @submit.prevent="submitForm">
            ...
            <div class="mb-3">
              ...
              <select class="form-select" id="segment" v-model="segment" required>
...
const { value: full_name, errorMessage: full_name_error } = useField("full_name");
const { value: email, errorMessage: email_error } = useField("email");
const { value: phone, errorMessage: phone_error } = useField("phone");
const { value: billing_address, errorMessage: billing_address_error } = useField("billing_address");
...
const submitForm = handleSubmit(async (values) => {
  try {
    loading.value = true;
    await api.put(`/customers/${props.data.id}`, values);
    emit("saved", true);
    closeModal();
  } catch (error) {
    console.error(error);
