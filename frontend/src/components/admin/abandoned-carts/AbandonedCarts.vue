<template>
  ...
  <!-- Export CSV Modal -->
  <div v-if="exportModalVisible" class="admin-modal modal fade show d-block" tabindex="-1" role="dialog" @click.self="closeExportModal">
...
const CSV_COLUMNS = [
  ...
  { label: 'Zip Code',            key: (r) => { const z = r._fd.zipcode; if (!z) return ''; return typeof z === 'object' ? (z.zipcode || '') : String(z); } },
  ...
  { label: 'Subtotal',            key: (r) => { const s = r._fd.subtotal; if (s == null) return ''; return typeof s === 'object' ? (s.subtotal != null ? parseFloat(s.subtotal).toFixed(2) : '') : parseFloat(s).toFixed(2); } },
